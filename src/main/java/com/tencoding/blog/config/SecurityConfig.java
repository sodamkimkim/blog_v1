...
@EnableGlobalMethodSecurity(prePostEnabled = true) // 특정 주소로 접근하면(post) 권한 및 인증 처리를 미리 체크 하겠다. 이사람이 미리 로그인되어있는지 안되어있는지 확인하고 안되어있으면 튕구고, 권한 없으면 글쓰기 못하고.
public class SecurityConfig extends WebSecurityConfigurerAdapter{ // 이거 extends해야 재정의 할 수 있다.
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable() // HttpSecurity하면 모든 녀석들 다 갖고 옴. 커스톰 하기 위해 제정의 하는 거임 여기서.
		.authorizeRequests()
		.antMatchers("/auth/**", "/","/js/**","/css/**","/image/**")//auth뒤에 달고 오는 애들은 다 허용해줄 거다. 이거 지정안해주면 static도 안됨.
		.permitAll()
		.anyRequest()
		.authenticated()
	.and()
		.formLogin()
		.loginPage("/auth/login_form");//위에서 허용해준 경로 말고 다른 경로로 오면,(인증된 사용자가 아니면) 로그인 페이지로 돌아가라고 처리 하는 거임.
